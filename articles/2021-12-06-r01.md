...
# カーネルのビルド  
どうやら`jetcard`のイメージは一部のカーネルパラメータが設定されていないみたいです。
```log
CONFIG_IP_NF_TARGET_REDIRECT: not set
CONFIG_AUFS_FS: not set - Required for aufs.
CONFIG_CGROUP_HUGETLB: not set - Required for hugetlb cgroup.
...
## 設定内容  
現在kubernetes 1.21.4が動作しているクラスタのカーネル設定も確認し、比較して作成。以下のような感じになりました。
```diff:diff
--- .config	2021-12-08 03:33:56.892746369 -0800
...
+CONFIG_DEVFREQ_GOV_POD_SCALING_HISTORY_BUFFER_SIZE_MAX=100
 ...
 CONFIG_EXTCON=y
```
初め`.config`を自分で書いたのですが、`make prepare`フェーズで補正する必要があったため、補正後のconfigでdiffをとっています。

楽をする場合は上をjetson上に上記ファイルを保存して
```sh
zcat /proc/config.gz > .config
patch -i diff
```
とすると、`.config`が上記の差分を当てた状態になります。
